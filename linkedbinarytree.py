...
class LinkedBinaryTree(BinaryTree):
    ...
    def toBST(self):
        ...
        # Get elements in the tree using inorder traversal
        elements = list(self.inorder())
        
        if len(elements) == 0: raise ValueError('cannot convert an empty tree')

        # Sort the elements
        elements.sort()

        # Reconstruct the BST using the sorted elements
        self._root = self._subtree_toBST(elements, 0, len(elements) - 1)

    def _subtree_toBST(self, elements, start, end):
        """Construct a sub BST recursively"""
        ...
        node = self._Node(elements[mid])

        # Construct the left and right subtrees recursively
        node._left = self._subtree_toBST(elements, start, mid - 1)
        if node._left is not None:
            node._left._parent = node
        node._right = self._subtree_toBST(elements, mid + 1, end)
        if node._right is not None:
